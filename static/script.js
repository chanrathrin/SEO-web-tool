/* ================================================================
   ...
   100% faithful port of Tkinter desktop behaviour
================================================================ */
'use strict';
...
// Image / crop state
let _origImage   = null;   // HTMLImageElement
let _origFile    = null;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (cw) {
    ...
    cw.addEventListener('drop', e => { e.preventDefault(); onCanvasDrop(e); });
  }
});
...
function showSpinner(msg) {
  document.getElementById('spinner-txt').textContent = msg || 'Processing…';
...
function toggleKeyVis() {
  ...
  inp.type = (cb && cb.checked) ? 'text' : 'password';
}
function syncKeyVis() { toggleKeyVis(); }
...
async function testKey() {
  ...
  try {
    ...
    if (d.ok) {
      apiStatus('✓ Key is valid', 'ok');
    } else {
      apiStatus('✗ ' + (d.error || 'Invalid key'), 'err');
    }
  } catch(e) {
    apiStatus('✗ Network error: ' + e.message, 'err');
  }
}
...
async function saveKey() {
  ...
  try {
    ...
    updateApiBadge();
    apiStatus('✓ Key saved & applied', 'ok');
    setTimeout(closeApiModal, 700);
    setStatus('API key loaded');
  } catch(e) {
    apiStatus('✗ ' + e.message, 'err');
  }
}
...
function sessionKey() {
  ...
  updateApiBadge();
  closeApiModal();
  setStatus('Session key active (not saved)');
}
...
function clearKey() {
  ...
  updateApiBadge();
  apiStatus('Saved key cleared', 'info');
}

function updateApiBadge() {
  const el = document.getElementById('api-badge');
  if (!el) return;
  if (_apiKey && _apiKeySaved) {
    el.textContent = '●  API key saved';     el.className = 'badge badge-ok';
  } else if (_apiKey) {
    el.textContent = '●  Session key active'; el.className = 'badge badge-warn';
  } else {
    el.textContent = '○  API not configured'; el.className = 'badge badge-off';
  }
}
...
async function processArticle() {
  ...
  try {
    ...
    // Store current_sections (mirrors desktop self.current_sections)
    _currentSections = {
      h1_copy:               d.h1       || '',
      intro_copy:            d.intro    || '',
    ...
    renderOutput(d.output_items || []);

    log('✓ Generate SEO completed successfully.');
    setStatus('SEO output generated ✓  —  Click 🤖 AI SEO Fields for AI-optimised Yoast fields');
  } catch(e) {
...
// Render output_items → output-display div
// Mirrors _render_output() with exact tag classes matching tag_configure colors
function renderOutput(items) {
  const div = document.getElementById('output-display');
  div.innerHTML = '';
  for (const item of items) {
    const span = document.createElement('span');
    // Map tag names to CSS classes (ot- prefix)
    span.className = 'ot-' + (item.tag || 'body');
    span.textContent = item.text || '';
    if (item.url) {
      const br = document.createElement('br');
      const a = document.createElement('a');
      a.href = item.url; a.target = '_blank'; a.className = 'embed-link';
      a.textContent = '    ↳ ' + (item.url.length > 80 ? item.url.slice(0,80) + '…' : item.url);
      span.appendChild(br); span.appendChild(a);
    }
    div.appendChild(span);
  }
}

// Quick Copy pills — exact match of copy_section() in desktop
function copySection(name) {
  if (!_currentSections || !Object.keys(_currentSections).length) {
...
function copyWpHtml() {
  const payload = _currentSections.wp_html_copy || '';
...
// ════════════════════ AI SEO POPUP ════════════════════════════
// Mirrors AISEOFieldsPopup exactly
...
function openAiSeoPopup() {
  ...
  // Clear all fields
  setVal('ai-fk', ''); setVal('ai-seo-title', ''); setVal('ai-meta', '');
...
async function runAiGenerate(articleText) {
  ...
  try {
    const r = await fetch('/api/ai-seo-fields', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({
        text: articleText,
        api_key: _apiKey,
    ...
    setVal('ai-fk', d.focus_keyphrase || '');
    updateFkCounter(d.focus_keyphrase || '');

    // Fill SEO Title (title1 is primary)
    const title1 = (d.seo_titles || [])[0] || '';
    setVal('ai-seo-title', title1);
    updateSeoCounter(title1);

    // Fill Meta Description
    const meta1 = (d.meta_descriptions || [])[0] || '';
    setVal('ai-meta', meta1);
    updateMetaCounter(meta1);

    // Render variants — matches _render_seo_variants / _render_meta_variants
    renderVariants('ai-seo-variants', d.seo_titles || [], 60, v => {
      setVal('ai-seo-title', v); updateSeoCounter(v);
    });
    renderVariants('ai-meta-variants', d.meta_descriptions || [], 160, v => {
      setVal('ai-meta', v); updateMetaCounter(v);
    });
...
function regenAiFields() {
  ...
  runAiGenerate(document.getElementById('article-input').value.trim());
}

// Apply to editor — mirrors _apply() in desktop
function applyAiFields() {
  const fk    = getVal('ai-fk').trim();
  const title = getVal('ai-seo-title').trim();
  const meta  = getVal('ai-meta').trim();
  if (!fk && !title && !meta) {
    alert('Empty Fields\n\nGenerate SEO fields first.'); return;
  }
  // Mirror _apply_ai_seo() — update current_sections
...
function copyAiField(id, isTextarea) {
...
// ── Character counters (exact match of desktop counter logic) ──

function updateFkCounter(val) {
  const words = val.trim() ? val.trim().split(/\s+/).length : 0;
  const chars = val.length;
  const el = document.getElementById('ai-fk-ctr');
  if (!el) return;
  if (!chars) { el.textContent = ''; el.className = 'ai-ctr'; return; }
  if (words <= 4) {
...
function updateSeoCounter(val) {
  const n = val.length; const MAX = 60;
  const pct = Math.min(100, (n / MAX) * 100);
  const bar  = document.getElementById('ai-seo-bar');
  const cnt  = document.getElementById('ai-seo-cnt');
  const hint = document.getElementById('ai-seo-hint2');
  if (!bar) return;
  bar.style.width = pct + '%';
  let col, hintText;
  if (!n)        { col = 'var(--txt-s)'; hintText = '—'; }
  else if (n < 30)  { col = 'var(--warn)'; hintText = `⚠ Too short (${n}/60)`; }
  else if (n < 50)  { col = 'var(--warn)'; hintText = `⚠ Could be longer (${n}/60)`; }
  else if (n <= MAX){ col = 'var(--ok)';   hintText = `✓ Good length (${n}/60)`; }
  else              { col = 'var(--bad)';  hintText = `✗ Too long — cut ${n-MAX} chars (${n}/60)`; }
  bar.style.background = col;
  cnt.textContent = n + ' / 60'; cnt.style.color = col;
  hint.textContent = hintText;
  hint.className = n <= MAX && n >= 50 ? 'ai-hint-ok' : n > MAX ? 'ai-hint-bad' : 'ai-hint-warn';
  updateGooglePreview();
}

function updateMetaCounter(val) {
  const n = val.trim().length; const MAX = 160;
  const pct = Math.min(100, (n / MAX) * 100);
  const bar  = document.getElementById('ai-meta-bar');
  const cnt  = document.getElementById('ai-meta-cnt');
  const hint = document.getElementById('ai-meta-hint2');
  if (!bar) return;
  bar.style.width = pct + '%';
  let col, hintText;
  if (!n)         { col = 'var(--txt-s)'; hintText = '—'; }
  ...
  else if (n <= MAX){ col = 'var(--ok)';   hintText = `✓ Acceptable (${n}/160)`; }
  else              { col = 'var(--bad)';  hintText = `✗ Too long — cut ${n-MAX} chars (${n}/160)`; }
  bar.style.background = col;
  cnt.textContent = n + ' / 160'; cnt.style.color = col;
  hint.textContent = hintText;
  hint.className = n >= 120 && n <= MAX ? 'ai-hint-ok' : n > MAX ? 'ai-hint-bad' : 'ai-hint-warn';
  updateGooglePreview();
}

function updateGooglePreview() {
  let title = getVal('ai-seo-title').trim() || 'SEO Title will appear here';
  let meta  = getVal('ai-meta').trim()      || 'Meta description will appear here…';
  if (title.length > 60) title = title.slice(0, 57) + '…';
  if (meta.length  > 160) meta  = meta.slice(0, 157) + '…';
  const gt = document.getElementById('gp-title');
  const gm = document.getElementById('gp-meta');
  if (gt) gt.textContent = title;
  if (gm) gm.textContent = meta;
}

// Render variant rows — mirrors _render_seo_variants / _render_meta_variants
function renderVariants(containerId, items, maxChars, onClickFn) {
  ...
  (items || []).slice(0, 3).forEach((v, i) => {
    ...
    if (!v) return;
    const n = v.length;
    const cntCls = n <= maxChars ? 'av-ok' : 'av-warn';
    const row = document.createElement('div');
    row.className = 'av-row';
    row.innerHTML =
      `<span class="av-num">${i+1}.</span>` +
      `<span class="av-txt">${escHtml(v.length > 120 ? v.slice(0,120)+'…' : v)}</span>` +
      `<span class="av-cnt ${cntCls}">${n}/${maxChars}</span>`;
    row.addEventListener('click', () => onClickFn(v));
...
function loadImageFile(file) {
  ...
  img.onload = () => {
    ...
    document.getElementById('crop-info').textContent =
      'Loaded: ' + file.name + '  •  ' + img.naturalWidth + '×' + img.naturalHeight + 'px';
...
function requestRedraw(delay) {
  if (_redrawId) clearTimeout(_redrawId);
  _redrawId = setTimeout(redraw, delay || 10);
}
...
const HLABELS = ['nw','n','ne','w','e','sw','s','se'];

function detectHandle(x, y) {
  const handles = getHandles();
  for (let i = 0; i < handles.length; i++) {
    const [hx, hy] = handles[i];
    if (Math.abs(x - hx) <= _handleSize && Math.abs(y - hy) <= _handleSize) return HLABELS[i];
  }
  return null;
}
function inCrop(x, y)  { const [x1,y1,x2,y2] = _cropRect; return x>=x1&&x<=x2&&y>=y1&&y<=y2; }
function inImage(x, y) {
  return x >= _imageOffsetX && x <= _imageOffsetX + _displayW &&
         y >= _imageOffsetY && y <= _imageOffsetY + _displayH;
}
...
function canvasDrag(e) {
  ...
  if (_draggingImage) {
    _imageOffsetX = _startOffset[0] + dx;
    _imageOffsetY = _startOffset[1] + dy;
  ...
  if (_draggingCrop) {
    if (_resizingHandle) {
      ...
      _cropRect = [x1+dx, y1+dy, x1+dx+w, y1+dy+h];
      clampCrop();
    }
    requestRedraw(5);
  }
...
function canvasRelease() {
  _draggingCrop = false; _draggingImage = false; _resizingHandle = null;
}
...
// Mirror _resize() from desktop exactly
function doResize(handle, dx, dy) {
  ...
  if (document.getElementById('lock-ratio').checked) {
    const r = _currentRatio;
    const w = Math.max(80, x2 - x1), h = Math.max(50, y2 - y1);
    if (Math.abs(dx) >= Math.abs(dy)) {
      const nh = w / r;
      if (handle.includes('n') && !handle.includes('s')) { y2 = y1 + nh; }
      else { y2 = y1 + nh; }
    } else {
      const nw = h * r;
      if (handle.includes('w') && !handle.includes('e')) { x1 = x2 - nw; }
      else { x2 = x1 + nw; }
    }
  }
...
// Apply crop — mirrors apply_crop() in desktop
function applyCrop() {
  if (!_origImage) { setStatus('Upload an image first'); return; }
  const [cx1, cy1, cx2, cy2] = _cropRect;
  const left   = Math.max(0, Math.round((cx1 - _imageOffsetX) / _displayScale));
  const top    = Math.max(0, Math.round((cy1 - _imageOffsetY) / _displayScale));
  const right  = Math.max(left + 1, Math.round((cx2 - _imageOffsetX) / _displayScale));
  const bottom = Math.max(top  + 1, Math.round((cy2 - _imageOffsetY) / _displayScale));

  const oc = document.createElement('canvas');
  oc.width  = right - left;
  oc.height = bottom - top;
...
// Export <100KB — sends to Flask /api/export-image which mirrors export_under_100kb()
async function exportUnder100kb() {
  ...
  const src = _croppedBlob
    ? new File([_croppedBlob], 'crop.jpg', {type:'image/jpeg'})
    : _origFile;
  fd.append('image', src);
  try {
    const r = await fetch('/api/export-image', {method:'POST', body: fd});
    if (!r.ok) {
      const d = await r.json();
...
// Generate image SEO — mirrors generate_seo() + _worker() in desktop
async function generateImageSeo() {
  const activeBlob = _croppedBlob || _origFile;
  ...
  fd.append('image',
    _croppedBlob
      ? new File([_croppedBlob], 'image.jpg', {type:'image/jpeg'})
      : _origFile
  );
  fd.append('api_key', _apiKey);
  fd.append('scene_notes', sceneNotes);

  try {
    const r = await fetch('/api/image-seo', {method:'POST', body: fd});
    const d = await r.json();
...
// Copy individual image field — mirrors copy_alt/copy_title/copy_caption in desktop
function copyImgField(field) {
  const MAP = {
    alt:     'img-alt',
    title:   'img-title-field',
    caption: 'img-caption',
  };
  const id = MAP[field];
  const el = document.getElementById(id);
  const val = el ? el.value.trim() : '';
  if (!val) { setStatus('No ' + field + ' to copy'); return; }
  clipboard(val);
  setStatus('Copied ' + (field === 'alt' ? 'Alt Text' : field === 'title' ? 'Image Title' : 'Caption'));
}

// Copy all image fields — mirrors copy_all() in desktop
...
// Clear all image fields — mirrors clear_fields() in desktop
function clearImageFields() {
  _origImage = null; _origFile = null; _croppedBlob = null;
